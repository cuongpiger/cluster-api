...
func (r *Reconciler) patchNode(ctx context.Context, remoteClient client.Client, node *corev1.Node, newLabels, newAnnotations map[string]string) error {
	...
	hasTaintChanges := taints.RemoveNodeTaint(newNode, clusterv1.NodeUninitializedTaint) || taints.RemoveNodeTaint(newNode, clusterv1.NodeCloudProviderUninitializedTaint)
